...
export class ProductsV1Controller {
  ...
  private static readonly LIST_CACHE: Map<
    string,
  ...
  async list(
    ...
    const rawSearch =
      typeof (filters as any).search === 'string'
        ? String((filters as any).search).trim()
        : '';
    ...
    if (Array.isArray((filters as any).categoryAlias))
      ids.push(...((filters as any).categoryAlias as number[]));
    if (Array.isArray((filters as any).categoriesCsv))
      ids.push(...((filters as any).categoriesCsv as number[]));
    const uniqIds = Array.from(
      new Set(ids.filter((n) => Number.isFinite(n) && n >= 1)),
    );
    if (uniqIds.length) {
    ...
    const items =
      useV2 && (norm as any).view === 'grid'
        ? result.items || []
        : (result.items || []).map(toProductCard);
    ...
    if (
      ...
      try {
        ...
        if (typeof dbg.usedOthers !== 'undefined')
          res.setHeader('X-Listing-Used-Others', String(dbg.usedOthers));
        if (typeof dbg.geoFilled !== 'undefined')
          res.setHeader('X-Listing-Geo-Filled', String(dbg.geoFilled));
        if (typeof dbg.fallbackToParent !== 'undefined')
          res.setHeader(
    ...
    if (
      ProductsV1Controller.LIST_CACHE.size >=
      ProductsV1Controller.LIST_CACHE_MAX
    ) {
      const firstKey = ProductsV1Controller.LIST_CACHE.keys().next().value as
        | string
        | undefined;
  ...
  @ApiQuery({
    name: 'categories',
    required: true,
    description: 'Comma-separated ids or slugs',
  })
  ...
  @ApiQuery({
    name: 'dedupe',
    required: false,
    description: 'If true, avoid duplicates across groups',
  })
  async categoryGroups(
    ...
    const perPage = Math.min(
      Math.max(parseInt(String(per || '8'), 10) || 8, 1),
      30,
    );
    const include =
      String(includeDescendants || 'true').toLowerCase() !== 'false';
    ...
    const groups = [] as Array<{
      key: string;
      items: ReturnType<typeof toProductCard>[];
    }>; // id or slug as key
  ...
  // Backwards-compatible detail endpoint for legacy mobile clients
  @Get(':id')
  @Header('Cache-Control', 'public, s-maxage=15, stale-while-revalidate=60')
  async detail(
