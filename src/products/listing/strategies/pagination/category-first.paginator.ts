...
export class CategoryFirstPaginator {
  ...
  async execute(
    ...
    geoAppend = false,
    opts?: { strict?: boolean },
  ): Promise<{ items: Product[]; total: number; meta?: { usedOthers: boolean; geoFilled: number } }> {
    const start = (page - 1) * perPage;

    const primaryQb = base.clone().andWhere('category.id IN (:...catIds)', { catIds: categoryIds });
    const othersQb = base.clone().andWhere('category.id NOT IN (:...catIds)', { catIds: categoryIds });
  ...
  usedOthers = otherItems.length > 0;
    ...
    if (geoAppend && !opts?.strict && items.length < perPage) {
      ...
      const filler = base
        .clone()
        .andWhere('product.id NOT IN (:...exclude)', { exclude: exclude.size ? Array.from(exclude) : [0] })
        .take(need);
    ...
    return { items, total: primaryTotal + othersTotal, meta: { usedOthers, geoFilled } };
  }
}
