import { BadRequestException, Body, Controller, Delete, Get, Headers, HttpCode, HttpStatus, Patch, Put, Query, Req, Res, UseGuards, UseInterceptors } from '@nestjs/common';
...
export class FavoritesController {
  ...
  @Get()
  async get(@Req() req: Request, @Res() res: Response, @Query('include') include?: string, @Headers('if-none-match') ifNoneMatch?: string) {
  ...
  @Patch()
  async patch(@Req() req: Request, @Res() res: Response, @Headers('if-match') ifMatch: string | undefined, @Query('merge') merge: 'union' | 'server' | 'client', @Body() body: PatchFavoritesDto) {
    const userId = (req as any).user?.id as number;
    const dto = body;
    const data = await this.favorites.patch(userId, dto, ifMatch, merge ?? 'union');
  ...
  @Put()
  async put(@Req() req: Request, @Res() res: Response, @Headers('if-match') ifMatch: string | undefined, @Body() body: PutFavoritesDto) {
    const userId = (req as any).user?.id as number;
