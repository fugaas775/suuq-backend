...
export class PhoneVerificationService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    if (!accountSid || !authToken || !this.serviceSid) {
      this.logger.warn(
        'Twilio Verify not fully configured (TWILIO_ACCOUNT_SID/TWILIO_AUTH_TOKEN/TWILIO_VERIFY_SERVICE_SID).',
      );
    } else {
  ...
  private normalizeE164(phone: string, region?: string): string {
    ...
    throw new BadRequestException(
      'Invalid phone number. Use local or E.164 (e.g., +251915333513).',
    );
  }
  ...
  async sendCode(
    ...
    const tryChannel = async (channel: 'sms' | 'whatsapp') => {
      return this.client!.verify.v2.services(
        this.serviceSid,
      ).verifications.create({ to: phone, channel });
    };
    const safe = (s: string) =>
      s.length > 6 ? s.slice(0, s.length - 6) + '******' : '******';
    try {
      ...
      this.logger.warn(
        `sendCode failed ch=${preferredChannel} to=${safe(phone)} code=${code} msg=${msg}`,
      );
      const looksBlocked =
        /blocked\s+for\s+the\s+SMS\s+channel/i.test(msg) ||
        /temporarily\s+blocked/i.test(msg);
      ...
      if (looksBlocked && canFallback) {
        try {
          ...
          return {
            ...
            channel: 'whatsapp',
            message:
              'SMS is blocked for this destination. We sent your code via WhatsApp instead.',
          };
        } catch (e2: any) {
          const msg2 = String(e2?.message || e2 || '');
          const code2 = e2?.code || e2?.status || '';
          this.logger.error(
            `fallback whatsapp failed to=${safe(phone)} code=${code2} msg=${msg2}`,
          );
          throw new BadRequestException(
            'SMS to this number is blocked and WhatsApp also failed. Please try a different phone number.',
          );
        }
      }
  ...
  async checkCode(
    ...
    this.ensureConfigured();
    const phone = this.normalizeE164(to, region);
    const res = await this.client!.verify.v2.services(
      this.serviceSid,
