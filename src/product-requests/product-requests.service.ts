  ...
  async listSellerFeed(
    ...
    if (!categoryIds && query.autoMatchSellerCategories !== false) {
      const rows = await this.productRepo
        ...
        .select('DISTINCT product.category_id', 'categoryId')
        .where('product.vendor_id = :sellerId', { sellerId })
    ...
    // Prefer requests that were explicitly forwarded to this vendor.
    qb.addOrderBy('forward.forwardedAt', 'DESC', 'NULLS LAST');
    qb.loadRelationCountAndMap('request.offerCount', 'request.offers');
