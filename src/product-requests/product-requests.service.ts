...
export class ProductRequestsService {
  ...
  async listSellerFeed(
    ...
    if (!categoryIds && query.autoMatchSellerCategories !== false) {
    ...
    if (categoryIds?.length) {
      qb.andWhere('request.category_id IN (:...categoryIds)', { categoryIds });
    }
  ...
  async listForwardedToSeller(
    ...
    const qb = this.requestRepo
      ...
      .andWhere('request.buyer_id <> :sellerId', { sellerId });
  ...
  async rejectOffer(
    ...
    if (body?.note) {
      offer.message = offer.message
        ? `${offer.message}\n---\nBuyer note: ${body.note}`
        : `Buyer note: ${body.note}`;
    }
    await this.offerRepo.save(offer);

    const pending = await this.offerRepo.count({
      where: {
        requestId,
        status: In([
          ProductRequestOfferStatus.SENT,
