...
export class ProductRequestsService {
  ...
  private async notifySuperAdminsOnCreate(request: ProductRequest) {
    ...
    for (const admin of admins) {
      ...
      await this.notifications.sendToUser({ userId: admin.id, title, body });
    }
  }
  ...
  async listSellerFeed(
    ...
    let categoryIds = query.categoryIds;
    
    // If no specific category filter is provided in the query, check user preferences or products
    if (!categoryIds) {
      const user = await this.userRepo.findOne({ 
        where: { id: sellerId }, 
        select: ['interestedCategoryIds'] 
      });

      if (user?.interestedCategoryIds?.length) {
        // PREFERENCE: Use the user's explicit interest list if set
        categoryIds = user.interestedCategoryIds;
      } else if (query.autoMatchSellerCategories !== false) {
        // FALLBACK: Use categories from their products
        const rows = await this.productRepo
          .createQueryBuilder('product')
          // Use actual column names generated by TypeORM (camelCase -> categoryId/vendorId)
          .select('DISTINCT product.categoryId', 'categoryId')
          .where('product.vendorId = :sellerId', { sellerId })
        ...
        if (!categoryIds?.length) categoryIds = undefined;
      }
    }
    ...
    const qb = this.requestRepo
      ...
      .leftJoin(
        'request.offers',
        'existingOffer',
        'existingOffer.seller_id = :sellerId AND existingOffer.status IN (:...offerStatuses)',
        {
          sellerId,
          offerStatuses: [
            ProductRequestOfferStatus.SENT,
      ...
      .leftJoinAndSelect(
        ...
        'forward',
        'forward.request_id = request.id AND forward.vendor_id = :sellerId',
        { sellerId },
      )
      .where('request.buyer_id <> :sellerId', { sellerId })
      // Order by primary key to avoid metadata resolution issues on date column
      .orderBy('request.id', 'DESC')
      .take(query.limit ?? 20);

    const activeStatuses = [ProductRequestStatus.OPEN];
    if (query.includeInProgress)
      activeStatuses.push(ProductRequestStatus.IN_PROGRESS);
    ...
    if (categoryIds?.length) {
      qb.andWhere(
        new Brackets((subQb) => {
          subQb
            .where('request.category_id IN (:...categoryIds)', { categoryIds })
            .orWhere('forward.id IS NOT NULL');
        }),
      );
    }
    ...
    qb.andWhere('existingOffer.id IS NULL');

    // Avoid ordering on forward to prevent TypeORM order-by metadata errors; rely on created_at ordering.
    qb.loadRelationCountAndMap('request.offerCount', 'request.offers');
