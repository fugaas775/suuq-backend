...
export class AdminCurationController {
  ...
  async getCuratedHome(@Query() q: any) {
    ...
    const categoryId = q.categoryId ? Number(q.categoryId) : (q.category ? Number(q.category) : undefined);
    ...
    else if (categoryId && Number.isInteger(categoryId)) baseFilters.categoryId = categoryId;

    const [curatedNew, curatedBest] = await Promise.all([
      this.productsService.findFiltered({ ...baseFilters, tags: curatedNewTags, sort: 'created_desc' }),
      this.productsService.findFiltered({ ...baseFilters, tags: curatedBestTags, sort: 'sales_desc' }),
    ]);
  ...
  @Post('home')
  async addCurations(@Body() body: { addNewIds?: number[]; addBestIds?: number[] }) {
    const addNewIds = Array.isArray(body?.addNewIds) ? body.addNewIds.filter((n) => Number.isInteger(n)) : [];
    const addBestIds = Array.isArray(body?.addBestIds) ? body.addBestIds.filter((n) => Number.isInteger(n)) : [];
    ...
    const summary: any = {
      ...
      notFoundNewIds: [],
      notFoundBestIds: [],
      errors: [] as Array<{ id: number; reason: string; bucket: 'new' | 'best' }>,
    };

    if (addNewIds.length) {
      const products = await this.productRepo.find({ where: { id: In(addNewIds) }, relations: ['tags'] });
      const foundIds = new Set(products.map((p) => p.id));
      const notFound = addNewIds.filter((id) => !foundIds.has(id));
      if (notFound.length) {
        summary.notFoundNewIds = notFound;
        for (const id of notFound) summary.errors.push({ id, reason: 'NOT_FOUND', bucket: 'new' });
      }
      for (const p of products) {
        ...
        if (!names.has(newTag.name)) {
          ...
          summary.errors.push({ id: p.id, reason: 'ALREADY_TAGGED', bucket: 'new' });
        }
      }
    }

    if (addBestIds.length) {
      const products = await this.productRepo.find({ where: { id: In(addBestIds) }, relations: ['tags'] });
      const foundIds = new Set(products.map((p) => p.id));
      const notFound = addBestIds.filter((id) => !foundIds.has(id));
      if (notFound.length) {
        summary.notFoundBestIds = notFound;
        for (const id of notFound) summary.errors.push({ id, reason: 'NOT_FOUND', bucket: 'best' });
      }
      for (const p of products) {
        const names = new Set((p.tags || []).map((t) => t.name));
        if (!names.has(bestTag.name)) {
          ...
          summary.errors.push({ id: p.id, reason: 'ALREADY_TAGGED', bucket: 'best' });
        }
      }
    }
    ...
    const [newCount, bestCount] = await Promise.all([
      this.productsService.findFiltered({ perPage: 1, tags: 'home-new,home_new,new_arrival,curated-new', sort: 'created_desc', view: 'grid' } as any).then(r => r.total),
      this.productsService.findFiltered({ perPage: 1, tags: 'home-best,home_best,best_seller,curated-best', sort: 'sales_desc', view: 'grid' } as any).then(r => r.total),
    ]);
    return { ...summary, curatedNewCount: newCount, curatedBestCount: bestCount };
  }

  // DELETE /api/admin/curation/home -> remove tags from product IDs
  @Delete('home')
  @HttpCode(HttpStatus.OK)
  async removeCurations(@Body() body: { removeNewIds?: number[]; removeBestIds?: number[] }) {
  ...
  @Post('home/remove')
  async removeCurationsPost(@Body() body: { removeNewIds?: number[]; removeBestIds?: number[] }) {
    return this._removeCurationsInternal(body);
  }

  private async _removeCurationsInternal(body: { removeNewIds?: number[]; removeBestIds?: number[] }) {
    const removeNewIds = Array.isArray(body?.removeNewIds) ? body.removeNewIds.filter((n) => Number.isInteger(n)) : [];
    const removeBestIds = Array.isArray(body?.removeBestIds) ? body.removeBestIds.filter((n) => Number.isInteger(n)) : [];
    ...
    if (removeNewIds.length) {
      const products = await this.productRepo.find({ where: { id: In(removeNewIds) }, relations: ['tags'] });
      const foundIds = new Set(products.map((p) => p.id));
      ...
      for (const p of products) {
        ...
        p.tags = (p.tags || []).filter((t) => t.name !== 'home-new' && t.name !== 'home_new' && t.name !== 'new_arrival' && t.name !== 'curated-new');
        if ((p.tags?.length || 0) !== before) {
          await this.productRepo.save(p);
          summary.removedNew.push(p.id);
        } else {
          summary.skippedNew.push(p.id);
          summary.errors.push({ id: p.id, reason: 'NOT_TAGGED', bucket: 'new' });
        }
      }
    }

    if (removeBestIds.length) {
      const products = await this.productRepo.find({ where: { id: In(removeBestIds) }, relations: ['tags'] });
      const foundIds = new Set(products.map((p) => p.id));
      ...
      for (const p of products) {
        const before = p.tags?.length || 0;
        p.tags = (p.tags || []).filter((t) => t.name !== 'home-best' && t.name !== 'home_best' && t.name !== 'best_seller' && t.name !== 'curated-best');
        if ((p.tags?.length || 0) !== before) {
          await this.productRepo.save(p);
          summary.removedBest.push(p.id);
        } else {
          summary.skippedBest.push(p.id);
          summary.errors.push({ id: p.id, reason: 'NOT_TAGGED', bucket: 'best' });
        }
      }
    }
  ...
  async patchProductTags(
    ...
    const add = (Array.isArray(body?.add) ? body.add : []).filter((t) => allowed.has(t));
    const remove = (Array.isArray(body?.remove) ? body.remove : []).filter((t) => allowed.has(t));

    const product = await this.productRepo.findOne({ where: { id }, relations: ['tags'] });
    if (!product) {
      throw new (require('@nestjs/common') as any).NotFoundException('Product not found');
    }
    ...
    if (remove.length) {
      ...
      for (const name of remove) if (!before.has(name)) skippedRemove.push(name);
    }
