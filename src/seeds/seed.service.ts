...
export class SeedService {
      ...
      if (!existing) {
        try {
          await this.categoriesService.create({
            name: slug.replace(/-/g, ' '),
            slug,
            iconName: desiredIcon,
          });
          this.logger.log(
            `Created category '${slug}' with icon '${desiredIcon}'.`,
          );
        } catch (e) {
          this.logger.warn(
            `Could not create category '${slug}': ${(e as Error).message}`,
          );
        }
      } else if (!existing.iconName) {
        await this.categoriesService.update(existing.id, {
          iconName: desiredIcon,
        });
        this.logger.log(
          `Backfilled iconName for '${slug}' -> '${desiredIcon}'.`,
        );
      }
    }
  ...
  async seedCategoriesFromJson() {
    ...
    for (const [parentName, meta] of Object.entries(data)) {
      ...
      const parentIconUrl = meta.iconName
        ? this.buildIconUrl(meta.iconName)
        : undefined;
      if (!parent) {
        parent = await this.categoriesService.create({
          name: parentName,
          slug: parentSlug,
        ...
        if (Object.keys(patch).length) {
          ...
          this.logger.log(
            `Backfilled parent '${parentName}' with ${JSON.stringify(patch)}`,
          );
        }
      }
      ...
      if (meta.subcategories?.length) {
        for (const sub of meta.subcategories) {
          ...
          const childIconUrl = sub.iconName
            ? this.buildIconUrl(sub.iconName)
            : undefined;
          if (!child) {
            child = await this.categoriesService.create({
              name: sub.name,
              slug: subSlug,
            ...
            this.logger.log(
              `Created child '${sub.name}' under '${parentName}'`,
            );
          } else {
            const patch: any = {};
            ...
            if (Object.keys(patch).length) {
              ...
              this.logger.log(
                `Updated child '${sub.name}' with ${JSON.stringify(patch)}`,
              );
            }
          }
        }
    ...
    for (const countryDto of countriesToSeed) {
      try {
        ...
        if (
          ...
          this.logger.warn(
            `Country "${countryDto.name}" already exists. Skipping.`,
          );
        } else {
