...
async function run() {
  ...
  const displayName = (argDisplayName || process.env.CREATE_DISPLAY_NAME || '').trim() || undefined;

  if (!email) {
    console.error('Usage: ts-node src/seeds/create-user.ts <email> <password> [displayName]');
