...
export class AdminThrottlerGuard extends ThrottlerGuard {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      const skip: boolean = SKIP_KEYS.some((k) =>
        reflector?.getAllAndOverride?.(k, [
          context.getHandler?.(),
          context.getClass?.(),
        ]),
      );
    ...
    const bypassPrefixes = (
      process.env.THROTTLE_BYPASS_PREFIXES ||
      '/api/health,/api/status,/api/vendors/products,/api/vendors/products/batch,/api/admin,/admin,/api/v1/products'
    )
      .split(',')
      .map((p) => p.trim().toLowerCase())
      .filter(Boolean);
    const bypassRoles = (
      process.env.THROTTLE_ADMIN_BYPASS_ROLES || 'ADMIN,SUPER_ADMIN'
    )
      .split(',')
