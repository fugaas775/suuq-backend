...
export class DirectUploadController {
  ...
  async requestSignedUrl(
    ...
    const putUrl = await this.doSpaces.getUploadSignedUrl(
      key,
      contentType,
      expiresIn,
    );
  ...
  async finalize(
    ...
    if (!exists) {
      throw new BadRequestException(
        'Uploaded object not found. Ensure the PUT to the signed URL completed successfully.',
      );
    }
    ...
    if (kind === 'image' || contentType.startsWith('image/')) {
      if (
        ...
        meta.contentLength > maxImageBytes
      ) {
      ...
      const mime =
        ft?.mime || resolvedContentType || 'application/octet-stream';
      ...
      const thumbUrl = await this.doSpaces.uploadFile(
        thumbBuffer,
        thumbKey,
        mime,
      );
      const lowResUrl = await this.doSpaces.uploadFile(
        lowResBuffer,
        lowResKey,
        mime,
      );
    ...
    if (kind === 'video' || contentType.startsWith('video/')) {
      ...
      this.generatePosterAsync(key, posterKey, maxVideoBytes).catch(
        () => void 0,
      );
      return {
        kind: 'video',
    ...
    if (kind === 'file' || contentType.startsWith('application/')) {
      ...
      if (!allowed.has(effectiveContentType)) {
        throw new BadRequestException(
          'Unsupported file type. Allowed: PDF, EPUB, ZIP.',
        );
      }
      if (
        typeof meta?.contentLength === 'number' &&
        meta.contentLength > maxFileBytes
      ) {
      ...
      if (deriveCover && effectiveContentType === 'application/pdf') {
        ...
        this.generatePdfCoverAsync(key, coverKey, maxFileBytes).catch(
          () => void 0,
        );
  ...
  async signedPlayback(
    ...
    const objectKey =
      key || (url ? this.doSpaces.extractKeyFromUrl(url) : undefined);
    ...
    const signedUrl = await this.doSpaces.getSignedUrl(objectKey, ttlSecs, {
      contentType: mime,
  ...
  private async generatePosterAsync(
    originalKey: string,
    posterKey: string,
    maxBytes: number,
  ): Promise<void> {
    try {
      ...
      const videoPath = path.join(
        tmpDir,
        `poster_${Date.now()}_${fileNameSafe}`,
      );
      ...
      const posterPath = path.join(
        tmpDir,
        `poster_${Date.now()}_${fileNameSafe}.jpg`,
      );
      const args = [
        '-y',
        '-hide_banner',
  ...
  private async generatePdfCoverAsync(
    originalKey: string,
    outputKey: string,
    maxBytes: number,
  ): Promise<void> {
