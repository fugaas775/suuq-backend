...
export class MulterExceptionFilter implements ExceptionFilter {
  ...
  log.warn(`File too large from ${req?.ip || 'unknown IP'}: ${exception?.message || exception}`);
  ...
  log.warn(`Multer error from ${req?.ip || 'unknown IP'}: ${exception?.code || exception?.message || exception}`);
  return res.status(HttpStatus.BAD_REQUEST).json({
        statusCode: HttpStatus.BAD_REQUEST,
        error: 'Upload error',
        message: 'There was a problem with the uploaded file. Please check the file type and try again.',
      });
    }
  ...
  log.warn(`Validation failed from ${req?.ip || 'unknown IP'}: ${exception?.message || exception}`);
  return res.status(HttpStatus.BAD_REQUEST).json({
        statusCode: HttpStatus.BAD_REQUEST,
        error: 'Invalid file',
