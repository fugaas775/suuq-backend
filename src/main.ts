...
async function bootstrap() {
  ...
  try {
    prometheusService = app.get(PrometheusService);
  } catch {
    // metrics module may be disabled in some environments
  }
  let legacyProductsRewriteWarned = false;
  ...
  // Backward-compat: rewrite legacy non-prefixed product detail endpoints
  // to the globally-prefixed API routes.
  app.use(
    (
      req: import('express').Request,
      ...
      if (!legacyProductsRewriteWarned) {
        legacyProductsRewriteWarned = true;
        bootstrapLogger.warn(
          'Legacy /products/* request detected and rewritten to /api/products/*. Track metric legacy_route_rewrites_total and remove this compatibility rewrite after client migration.',
        );
      }
  ...
  app.use(
    (
      ...
      if (isProduction) {
        // Structured logging in production with stronger sampling on known noisy endpoints.
        const urlPath = String(req.url || '').split('?')[0];
        const isNoisyEndpoint =
          urlPath === '/api/products/impressions' ||
          urlPath === '/api/v1/products/likes';
        // Default sample rate: 1/N. Override via LOG_SAMPLE_N and LOG_NOISY_SAMPLE_N.
        const defaultSampleN = Math.max(
          1,
          parseInt(process.env.LOG_SAMPLE_N || '5', 10),
        );
        const noisySampleN = Math.max(
          1,
          parseInt(process.env.LOG_NOISY_SAMPLE_N || '50', 10),
        );
        const sampleN = isNoisyEndpoint ? noisySampleN : defaultSampleN;
