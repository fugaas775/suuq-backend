  ...
  private async handleBroadcast(opts: BroadcastJobData) {
    ...
    const notifications = users.map((u) =>
      this.notificationRepository.create({
        ...
        body: opts.body,
        type,
        data: { ...(opts.data || {}), image: opts.image },
      }),
    );
    ...
    // Inject 'type' so Flutter client routing works reliably
    fcmData['type'] = type;

    const messageBase = {
      notification: { title: opts.title, body: opts.body, image: opts.image },
      data: fcmData,
    };
    ...
    if (messaging && messaging.sendEachForMulticast) {
      this.logger.log(
        `Starting FCM broadcast to ${uniqueTokens.length} tokens`,
      );
    ...
    this.logger.log(
      `Broadcast complete. Success: ${successCount}, Failed: ${failureCount}, Throttled: ${throttledCount}`,
    );
    return { successCount, failureCount };
  }

  private async pruneInvalidTokens(response: any, tokens: string[]) {
    ...
    this.logger.warn(`Pruned ${invalidTokens.length} invalid device tokens`);
  }
}
